...
gsl_spline *spline[25]; // Used for interpolation
...
{
    if (r > 0 && r <= 129)
        return gsl_spline_eval(spline[24], r, acc[24]);
    else
        return 0;
}
...
void write_R1() {
    ...
    char name_tmp[BUFSIZ];
    strcpy(name_tmp, outputdir);
    strcat(name_tmp, "/func_R1.dat");
    f = fopen(name_tmp, "w+");
...
void write_Qn() {
    ...
    printf("getting Q function 1/8...\n");
    char name_tmp[BUFSIZ];
    strcpy(name_tmp, outputdir);
    strcat(name_tmp, "/func_Q1.dat");
    f = fopen(name_tmp, "w+");
    for (int i = 0; i <= nk; i++) {
        double k = pow(10, log_kmin + i * dk);
        Q1_array[i] = Q_n(1, k);
    ...
    printf("getting Q function 2/8...\n");
    strcpy(name_tmp, outputdir);
    strcat(name_tmp, "/func_Q2.dat");
    f = fopen(name_tmp, "w+");
    for (int i = 0; i <= nk; i++) {
        double k = pow(10, log_kmin + i * dk);
        Q2_array[i] = Q_n(2, k);
    ...
    printf("getting Q function 5/8...\n");
    strcpy(name_tmp, outputdir);
    strcat(name_tmp, "/func_Q5.dat");
    f = fopen(name_tmp, "w+");
    for (int i = 0; i <= nk; i++) {
        double k = pow(10, log_kmin + i * dk);
        Q5_array[i] = Q_n(5, k);
    ...
    printf("getting Q function 8/8...\n");
    strcpy(name_tmp, outputdir);
    strcat(name_tmp, "/func_Q8.dat");
    f = fopen(name_tmp, "w+");
    for (int i = 0; i <= nk; i++) {
        double k = pow(10, log_kmin + i * dk);
        Q8_array[i] = Q_n(8, k);
...
void q_function() {
    ...
    char name_tmp[BUFSIZ];
    strcpy(name_tmp, outputdir);
    strcat(name_tmp, "/Xi_L_func.dat");
    fxi = fopen(name_tmp, "w+");
    fprintf(fxi, "%.13lf %.13lf\n", 0., Xi_L(0.));
    fclose(fxi);

    fxi = fopen(name_tmp, "a+");
...
void M_0(double y, double R, double M0_fin[]) {
    ...
    for (l = 0; l < 6; l++) // Initialise at each step
        M0_tab[l] = 0;
...
void M_1(double y, double R, double M1_fin[]) {
    ...
    for (mu = -1; mu <= 1; mu += dmu) { // Decomp integrale dtheta en somme de riemann int=somme f(x_i)*delta_x
        ...
        for (l = 0; l < 3; l++) {
            for (m = 0; m < 3; m++) {
                for (o = 0; o < 3; o++)
                    s += G[l][m] * U[l] * A_dot[m][o] * rn[o]; // Corrected typo from Wang14, See White14
            }
        }
        F_b[1] -= 2. * s;
...
void write_Sigma() {
    ...
    char name_tmp[BUFSIZ];
    strcpy(name_tmp, outputdir);
    strcat(name_tmp, "/Sigma_12_CLPT.dat");
    fi = fopen(name_tmp, "w+");
    for (r = 1; r <= r_max; r += 1) {
